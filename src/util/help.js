...
const DEFAULT_SECTIONS = [
  { name: '💳 金流操作（客服／管理員）',
    value: '`!結帳 @老闆 陪玩 原價 [折抵]` — 選券→選付款方式（現金／雨幣）→發結帳明細\n'
         ...
         + '`!分潤 @陪玩 85` — 調整個別陪玩的分潤成數（限管理員）；`!分潤 @陪玩 預設` 改回跟隨全域，'
         + '不帶數字＝查詢' },
  ...
  { name: '🎰 BINGO（雨果幣）',
    value: '`/bingo 下注金額` — 5×5 連線遊戲，最低 500 雨果幣；結果下方可再玩一次或雙倍\n'
         + '12 種圖案隨機生成，5 個相同並列（直／橫／斜）就是一條線；**全盤同圖案是頭獎，全服只有一份**\n'
         + '中獎請截圖結果找客服兌換獎勵（不會自動發回雨果幣）\n'
         + '`!查詢雨果幣 [@老闆]` — 查餘額與儲值／扣款紀錄，查自己免權限\n'
         + '`/儲值雨果幣` `/扣款雨果幣` `/兌換bingo 局號` `!雨果幣總覽`（客服）\n'
         + '（雨果幣只能由客服手動儲值，與雨幣分開，不影響點單消費與 VIP 累計）' },

  { name: '🔮 喚雨星象（全員）',
    value: '`!抽籤` / `/抽籤` — 運勢抽籤，不限次數；獎池裡若設有折價券，抽中會進背包\n'
